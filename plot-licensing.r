...
p2 <- ggplot(l) + aes(x = license) + geom_bar()

p3 <- ggplot(subset(l, portal == 'data.sfgov.org' | portal == 'data.cityofnewyork.us' | portal == 'data.cityofchicago.us' | portal == 'data.gov.uk' | portal == 'explore.data.gov')) +
  facet_wrap(~ portal) + aes(x = license, fill = portal_software) + geom_bar() +
  coord_flip()
